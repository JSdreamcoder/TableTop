    ...
    <>
      {!loading && (
        ...
        >
          ...
          <img
            src="https://img.freepik.com/free-vector/email-messaging-email-marketing-campaign_183665-8.jpg?w=2000"
            width="300"
          ></img>
          <p class="text-muted mb-4">
            An Email has been send to{" "}
            <a>
              <b>{props.CurrentUser.email}</b>
            </a>{" "}
          ...
          <div class="d-flex justify-content-between">
            <button
              class="btn btn-primary general-btn height-10px"
              disabled={timer !== 0}
              onClick={resetTimer}
            >
              Resend Email {timer > 0 && <>(00:{timer})</>}
            </button>
            <button
              class="btn btn-primary ghost-btn general-btn height-10px"
              onClick={() =>
                window.open(providerLink, "_blank", "noopener,noreferrer")
              }
            >
              {validProvider ? "Go To Email" : "Contact Support"}
            </button>
          </div>
          <p className="text-2 text-dark mt-3">
            Activate email later?{" "}
            <Link className="fw-500" to="/">
              Go Back
            </Link>
          </p>
        </motion.div>
